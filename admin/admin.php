    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
            ...
            <div class="navbar-nav mt-3">
                <a class="nav-link active me-3 btn btn-outline-primary btn-rounded text-wrap" style="color:white ;" aria-current="page" href="#">
                    <h4>Home</h4>
                </a>
...
<div class="bg-container">
    <body>
        <h2 class="text-center pt-3">Reports</h2>
        <div class="col-lg-4 bg-white m-auto mt-5 p-3">
            <form action="" method="POST">
                <h2 class="text-center mb-5">Enter Begin and End Date</h2>
                <div class="input-group gap-4 mb-3 d-flex justify-content-center">
        ...
        <button class="btn btn-success ms-5 mt-3" id="btn-print" onclick="printTable()">Print Table</button>
        </div>
            ...
            <div class="row-md-5">
                <table class="table table-striped table-bordered table-hover mt-5">
                    ...
                    <tbody>
                        ...
                        ?>
                                <h1 class="text-center" id="report-name"> Reports From <?php echo "$begindate" ?> to
                                    <?php echo "$enddate" ?></h1>
                                <?php
                                ...
                                if (mysqli_num_rows($select_date) > 0) {
                                    while ($row = mysqli_fetch_assoc($select_date)) {
                                        ...
                                        $customer_queryid = "SELECT studid FROM student_record WHERE SR_Code = '$srcode'";
                                        $view_customerid = mysqli_query($conn, $customer_queryid);
                                        if (mysqli_num_rows($view_customerid) > 0) {
                                            while ($rowid = mysqli_fetch_assoc($view_customerid)) {
                                                $id = $rowid['studid'];
                                                $customer_query = "SELECT * FROM tbstudinfo WHERE studid = '$id'";
                                                ...
                                                if (mysqli_num_rows($view_customer) > 0) {
                                                    while ($customer_row = mysqli_fetch_assoc($view_customer)) {
                                                        ...
                                                        $department = $customer_row['course'];
